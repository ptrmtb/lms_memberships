...
def get_meta_from_document(app_path):
	# Handle membership page
	if app_path == "membership":
		return {
			"title": _("Membership Plans"),
			"keywords": "Membership, Subscription, Plans, Access",
			"link": "/membership",
			"description": _("Subscribe to get unlimited access to all courses"),
		}
	...
	if re.match(r"^batches/details/.*$", app_path):
		...
		if batch and batch.batch_details:
	...
	if re.match(r"^job-openings/.*$", app_path):
		...
		if job_opening and job_opening.description:
